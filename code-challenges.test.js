...
//I think im going to use map so i can go around my array the same size.
...
//Im going to split my name so i can make my index 0 . toUpperCase and after that im going to use .join.


const sentenceCap =(array, value) => {
  return array.map(value => {
    let newCase = value.name.split(" ").map(value => value[0].toUpperCase() + value.slice(1)).join(" ")
     return `${newCase} is ${value.occupation}`
  })
}
//So my test fail
// I had trouble with this one i got stuck i ask for help but i still cant make it work :(
...
const remainderDiv = (array) => {
  ...
  for(let i=0; i < array.length; i++){
   if(typeof array[i] === `number` && divRemain.push(array[i]%3))
  }
      return divRemain
}
...
//I will be returning to fix all my errors. :(, I am still getting that i receive 99 istead of [99] and i try to use .split and after that another method to change from number to string to array and is just not working HELP.
//I back and im still not able to make this pass I dont know why im having a hard time with this assessment i been stuck here for a while so im going to live it like this.
// Expected output: 99
